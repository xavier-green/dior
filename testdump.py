...
tables=[ 'LOCA_location', 'COLO_color', 'COUN_country', 'DEPT_department', 'DIVI_division' , 'FAMI_family', 'GROU_group', 'MATE_material', 'ITEM_item', 'SHAP_shape', 'STAT_state', 'SZONE_sub_zone', 'THEM_theme', 'UZONE_zone', 'ZONE_zone', 'COLL_Collection' ]
files=['Boutiques', 'color', 'country', 'Departements', 'Divisions', 'family', 'Groupe', 'material', 'products', 'shape', 'state', 'szone', 'Themes', 'uzone', 'zone', 'collection']
...
for index, x in enumerate(tables):
    ...
    if x == 'COLL_Collection':
        tot = tot[:-1] + " where COLL_Description not like '%collection%'" + '"'
